...
async def obtener_usuario_lider(usuario_actual: dict = Depends(obtener_usuario_actual)) -> dict:
    if not usuario_actual.get("es_lider", False):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="No tienes permisos de líder para realizar esta acción"
        )
